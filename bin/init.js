...
export async function init(options) {
  ...
  const answers = await inquirer.prompt([
    ...
    },
    {
      ...
      when: (answers) =>
        (answers.framework || options.framework) !== "mintlify",
  ...
  try {
    ...
    if (answers.setupGitHubActions) {
      console.log(
        `  ${
          options.skipInstall ? "2" : "1"
        }. Vai su GitHub → Settings → Secrets → Actions`
      );
      console.log(
        `  ${options.skipInstall ? "3" : "2"}. Aggiungi secret ${chalk.yellow(
          "OPENROUTER_API_KEY"
        )} con la tua API key`
      );
      console.log(
        `  ${
          options.skipInstall ? "4" : "3"
        }. Abilita "Allow GitHub Actions to create PRs" in Settings → Actions → General`
      );
    }

    console.log(
      `  ${
        answers.setupGitHubActions
          ? options.skipInstall
            ? "5"
            : "4"
          : options.skipInstall
          ? "2"
          : "1"
      }. ${chalk.cyan("npx auto-docs generate")} per generare documentazione`
    );
    console.log(
      `  ${
        answers.setupGitHubActions
          ...
          ? "3"
          : "2"
      }. ${chalk.cyan("npx auto-docs dev")} per avviare il dev server\n`
    );

    if (framework === "mintlify") {
      console.log(
        chalk.blue(
          "💡 Per Mintlify, dovrai anche configurare il deploy su mintlify.com"
        )
      );
    } else if (answers.deploy === "github-pages") {
      console.log(
        chalk.blue(
          "💡 GitHub Pages sarà configurato automaticamente al primo push"
        )
      );
    }
