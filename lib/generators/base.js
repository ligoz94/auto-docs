...
export class BaseGenerator {
  ...
  async analyze(options = {}) {
    ...
    try {
      ...
      // Git empty tree hash (represents the state before any commits)
      const emptyTree = '4b825dc642cb6eb9a060e54bf8d69288fbee4904';

      // Get diff
      let diffSummary;
      if (force) {
        // Get all files from beginning of git history
        diffSummary = await git.diff(['--name-status', `${emptyTree}..HEAD`]);
      } else {
        ...
        if (lastCommit) {
          ...
          // First run - get all files from beginning
          diffSummary = await git.diff(['--name-status', `${emptyTree}..HEAD`]);
        }
      }
      ...
      for (const line of lines) {
        ...
        // Get file diff (use empty tree for comparison to show full file content)
        const diff = await git.diff([`${emptyTree}`, '--', filePath]);
